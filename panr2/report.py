...
def write_report(output_dir, base_name, options=None, panr2_version="unknown", input_files=None, feature_outputs=None):
    ...
    if feature_outputs:
        ...
        for feature_type in sorted(feature_outputs):
            for rel_path in [
                f"{feature_type}/{feature_type}_merged.csv",
                f"{feature_type}/{feature_type}_tidy.csv",
                f"{feature_type}/{feature_type}_feature_summary.csv",
                f"{feature_type}/{feature_type}_category_summary.csv",
                f"{feature_type}/{feature_type}_sample_burden.csv",
            ]:
                lines.append(f"- `{rel_path}`")
        lines.append("")
    ...
    lines.append(
        ...
        "Static and interactive visualizations were generated from the same filtered data tables. Optional VFDB and PlasmidFinder analyses, when provided, were parsed as separate feature families and summarized independently from AMR resistance classes."
    )
    lines.append("")
